/**
 * Testimonial Data — Lumière Visual Studio
 * Client reviews displayed in a carousel on the LandingPage.
 * 9 testimonials shown 3 per slide.
 * Used by: LandingPage (Testimonials section)
 */

export const testimonialData = [
  {
    // img: 'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=400&q=80',
    name: 'Sofia Marchetti',
    role: 'Travel Blogger, Rome',
    text: 'Absolutely breathtaking work. Every shot tells a story we didn\'t know we were living.'
  },
  {
    // img: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=400&q=80',
    name: 'James Holloway',
    role: 'Creative Director, London',
    text: 'The team\'s eye for light and composition is unmatched. Our brand campaign exceeded expectations.'
  },
  {
    // img: 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=400&q=80',
    name: 'Amara Diallo',
    role: 'Wedding Client, Paris',
    text: 'From booking to delivery, the experience was seamless. The photos are simply magical.'
  },
  {
    // img: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=400&q=80',
    name: 'Marcus Chen',
    role: 'E-commerce Founder, Singapore',
    text: 'Incredible attention to detail. Our product shots have driven a 40% uplift in conversions.'
  },
  {
    // img: 'https://images.unsplash.com/photo-1487412720507-e7ab37603c6f?w=400&q=80',
    name: 'Lena Bauer',
    role: 'Hospitality Owner, Bavaria',
    text: 'They captured the soul of our landscape retreat better than we imagined possible.'
  },
  {
    // img: 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=400&q=80',
    name: 'Ravi Shankar',
    role: 'Documentary Producer, Mumbai',
    text: 'Professional, punctual and extraordinarily talented. Highly recommend for any project.'
  },
  {
    // img: 'https://images.unsplash.com/photo-1580489944761-15a19d654956?w=400&q=80',
    name: 'Isabelle Fontaine',
    role: 'NGO Director, Geneva',
    text: 'The portrait series they created for our foundation gala was deeply moving and powerful.'
  },
  {
    // img: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=400&q=80',
    name: 'Tomás Ferreira',
    role: 'Architect, Lisbon',
    text: 'Working with Lumière transformed how we see and share our architectural projects.'
  },
  {
    // img: 'https://images.unsplash.com/photo-1521119989659-a83eee488004?w=400&q=80',
    name: 'Priya Nair',
    role: 'Fashion Designer, Delhi',
    text: 'From the first call to the final files — an absolute pleasure. Will work with them forever.'
  },
  {
    // img: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=400&q=80',
    name: 'Elena Petrova',
    role: 'Interior Designer, Moscow',
    text: 'Every frame felt curated. They understood our aesthetic before we even explained it.'
  },
  {
    // img: 'https://images.unsplash.com/photo-1499996860823-5214fcc65f8f?w=400&q=80',
    name: 'David Okafor',
    role: 'Startup Founder, Lagos',
    text: 'Our investor deck photos came from this shoot. Genuinely elevated how people see the brand.'
  },
  {
    // img: 'https://images.unsplash.com/photo-1544723795-3fb6469f5b39?w=400&q=80',
    name: 'Hana Kobayashi',
    role: 'Restaurant Owner, Kyoto',
    text: 'The food and interior shots doubled our online reservations within a month.'
  },
  {
    // img: 'https://images.unsplash.com/photo-1601412436009-d964bd02edbc?w=400&q=80',
    name: 'Lucas Meyer',
    role: 'Event Planner, Berlin',
    text: 'Calm, fast, unobtrusive on the day — and the gallery they delivered was flawless.'
  },
  {
    // img: 'https://images.unsplash.com/photo-1524504388940-b1c1722653e1?w=400&q=80',
    name: 'Aisha Rahman',
    role: 'Bride, Dhaka',
    text: 'I cried looking through our wedding gallery. They caught moments we didn\'t even see happen.'
  },
  {
    // img: 'https://images.unsplash.com/photo-1560250097-0b93528c311a?w=400&q=80',
    name: 'Nathan Cole',
    role: 'Musician, Nashville',
    text: 'Album cover and tour photos in one session. Efficient, creative, worth every rupee.'
  }
  
];

