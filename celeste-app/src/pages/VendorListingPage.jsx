...
const EP_STYLES = `
  /* All neutral surfaces/text/borders below use the app's shared theme
     tokens (same ones VendorListingPage.css consumes) instead of
     hardcoded light-mode hex values, so this panel blends into both
     dark and light theme instead of always rendering as a white card.
     Accent hexes (#534AB7 purple for demo/static vendors, #D4860A amber
     for real DB vendors) are intentionally kept as fixed brand colors —
     they read fine on both themes and preserve the existing distinction
     between the two vendor sources. */
  .ep-wrap { background:var(--bg-card); border-radius:16px; border:1px solid var(--border-medium); overflow:hidden; margin-bottom:24px; animation:epIn 0.28s cubic-bezier(0.22,1,0.36,1); box-shadow:0 8px 40px rgba(0,0,0,0.22); }
  @keyframes epIn { from{opacity:0;transform:translateY(-12px)} to{opacity:1;transform:translateY(0)} }
  .ep-img-col { position:relative; overflow:hidden; min-height:520px; background:var(--bg-elevated); }
  ...
  .ep-right { padding:28px 32px; display:flex; flex-direction:column; justify-content:space-between; background:var(--bg-card); overflow-y:auto; }
  .ep-name { font-family:'Cormorant Garamond',serif; font-size:30px; font-weight:400; color:var(--text-primary); margin-bottom:6px; line-height:1.2; }
  ...
  .ep-cell { background:var(--bg-elevated); border-radius:10px; padding:11px 14px; border:0.5px solid var(--border-soft); }
  .ep-cell-label { font-size:9px; letter-spacing:1.1px; text-transform:uppercase; color:var(--amber-bright, #ecea3d);; margin-bottom:4px; font-weight:600; }
  .ep-cell-val { font-size:14px; color:var(--text-primary); font-weight:500; }
  .ep-types { display:flex; flex-wrap:wrap; gap:7px; margin-bottom:18px; }
  .ep-type-pill { font-size:12px; padding:5px 14px; border-radius:20px; background:rgba(83,74,183,0.12); color:#8a80e8; font-weight:500; border:0.5px solid rgba(83,74,183,0.28); }
  .ep-tags { display:flex; flex-wrap:wrap; gap:7px; margin-bottom:16px; }
  .ep-award-pill { font-size:11px; padding:4px 12px; border-radius:20px; background:var(--bg-elevated); color:var(--text-muted); border:0.5px solid var(--border-soft); }
  .ep-footer { display:flex; align-items:center; justify-content:space-between; gap:10px; flex-wrap:wrap; margin-top:auto; padding-top:16px; border-top:0.5px solid var(--border-soft); }
  .ep-price { display:flex; align-items:baseline; gap:2px; }
  .ep-price-sym { font-size:15px; color:var(--text-primary); }
  .ep-price-val { font-size:26px; font-weight:500; color:var(--text-primary); font-family:'Cormorant Garamond',serif; }
  .ep-price-unit { font-size:12px; color:var(--text-faint); margin-left:4px; }
  ...
  .ep-cta:hover { background:#443c9c; }
  .ep-cta-amber { background:var(--amber-primary, #D4860A) !important; color:var(--bg-base, #0F0D0A) !important; }
  .ep-cta-amber:hover { background:var(--amber-bright, #b86d08) !important; }
  .ep-related-label { border-top:0.5px solid var(--border-soft); padding:14px 20px 10px; font-size:10px; letter-spacing:1.2px; text-transform:uppercase; color:var(--text-faint); font-weight:600; background:var(--bg-card); }
  .ep-related-grid { display:grid; grid-template-columns:repeat(5,1fr); gap:10px; padding:0 16px 16px; background:var(--bg-card); }
  .ep-rel-card { border-radius:10px; overflow:hidden; cursor:pointer; aspect-ratio:4/3; position:relative; transition:transform 0.15s; background:var(--bg-elevated); }
  ...
  .ep-verified-badge { display:inline-flex; align-items:center; gap:4px; font-size:10px; font-weight:500; color:#8a80e8; background:rgba(83,74,183,0.12); padding:3px 9px; border-radius:20px; }
  .ep-specialty-badge { display:inline-flex; align-items:center; gap:4px; font-size:10px; font-weight:500; color:var(--amber-bright, #D4860A); background:rgba(212,134,10,0.12); padding:3px 9px; border-radius:20px; border:0.5px solid rgba(212,134,10,0.3); }
  ...
  @media (max-width: 900px) {
    .ep-top { grid-template-columns: 38% 1fr; min-height: 420px; }
    .ep-img-col { min-height: 420px; }
    .ep-right { padding: 22px 24px; }
    .ep-related-grid { grid-template-columns: repeat(3, 1fr); }
  }
  @media (max-width: 720px) {
    .ep-top { grid-template-columns: 1fr; min-height: 0; }
    .ep-img-col { min-height: 300px; height: 300px; }
    .ep-right { padding: 20px; }
    .ep-name { font-size: 24px; }
    .ep-grid { grid-template-columns: 1fr 1fr; gap: 8px; }
    .ep-footer { flex-direction: column; align-items: stretch; }
    .ep-footer .ep-price { justify-content: center; }
    .ep-cta { width: 100%; text-align: center; }
    .ep-related-grid { grid-template-columns: repeat(2, 1fr); }
  }
  @media (max-width: 420px) {
    .ep-img-col { min-height: 220px; height: 220px; }
    .ep-right { padding: 16px; }
    .ep-name { font-size: 21px; }
    .ep-grid { grid-template-columns: 1fr; }
    .ep-related-grid { grid-template-columns: repeat(2, 1fr); padding: 0 12px 12px; }
    .ep-carousel-nav { width: 30px; height: 30px; }
    .ep-close, .ep-bm { width: 34px; height: 34px; }
  }
`;
...
export default function VendorListingPage({ bookmarks, onBookmarkToggle, serviceConfig = DEFAULT_VENDOR_SERVICE }) {
  ...
  // Controls whether the filter sidebar renders as an open slide-in
  // drawer on small screens (<=900px). Ignored on desktop — the
  // wrapper there is `display: contents` so the sidebar just sits
  // inline in .page-body as before.
  ...
  return (
    <div className="photo-page">
      {pickContext && (
        <div style={{
          background: '#D4860A', color: '#0F0D0A', padding: '10px 16px',
          fontSize: 13, fontWeight: 600, display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8,
      ...
      <div className="photo-page-header">
        <div className="photo-page-header-inner">
          ...
          <div className="photo-page-header-right">
            ...
            {/* Mobile: Filters + Sort render as two side-by-side pills.
                Desktop: display:contents keeps this invisible to layout. */}
      ...
      <div className="page-body">
        {/* Backdrop only renders (and is only visible) on mobile, when open */}
        {mobileFiltersOpen && (
          <div className="filter-backdrop" onClick={() => setMobileFiltersOpen(false)} />
        )}

        <div className={`filter-drawer ${mobileFiltersOpen ? 'filter-drawer-open' : ''}`}>
          ...
          <div className="filter-drawer-close">
            <span className="filter-drawer-title">
              Filters
              {activeFilterCount > 0 && <span className="filter-drawer-badge">{activeFilterCount}</span>}
            </span>
            <div className="filter-drawer-actions">
              {activeFilterCount > 0 && (
                <button type="button" className="filter-drawer-clear" onClick={clearAll}>
                  Clear all
                </button>
              )}
              <button
                type="button"
                className="filter-drawer-x"
        ...
        >
          ...
          {/* Mobile-only "apply" button — same pattern as ExplorePage/Sidebar,
              closes the drawer to reveal the already-live-filtered results. */}
          <button type="button" className="filter-drawer-apply" onClick={() => setMobileFiltersOpen(false)}>
            Show results
          </button>
        </div>
