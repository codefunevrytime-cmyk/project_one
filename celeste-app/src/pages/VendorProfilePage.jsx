...
export default function VendorProfilePage({ bookmarks, onBookmarkToggle, serviceConfig = DEFAULT_VENDOR_SERVICE }) {
 ...
 const staticPhotographer = null; // resolved after DB fetch
  ...
  const [messageSending] = useState(false);
...
const photographer = dbVendor
  ...
  return (
    <div className="pp-page">
      ...
      {/* Booking Modal */}
      {showBookingModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 'clamp(12px, 4vw, 24px)', boxSizing: 'border-box' }} onClick={() => setShowBookingModal(false)}>
          <div style={{ background: '#1e1a14', border: '0.5px solid rgba(200,175,120,0.2)', borderRadius: 16, padding: 'clamp(20px, 5vw, 32px)', maxWidth: 420, width: '100%', boxSizing: 'border-box', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.6)' }} onClick={e => e.stopPropagation()}>
