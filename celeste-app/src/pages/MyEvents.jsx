...
/* ─── Status config ──────────────────────────────────────────────────────────
   Every status the client can actually see, each with its own real label —
   no folding/hiding here. 'pending' means "we got it, nobody's looked yet"
   and 'admin_reviewing' means "someone's actively looking at it now" — that
   distinction matters to the client even though the admin panel collapses
   it (admin doesn't need a separate label for an unopened card sitting in
   their own list — see AdminEventRequests.jsx). The moment admin actually
   opens the card, the real DB status flips pending → admin_reviewing (via
   AdminEventRequests.jsx's auto-PATCH-on-open), so this label change
   reflects a real state change, not just a display trick. */
const STATUS_CFG = {
  pending:           { label: "Submitted",              color: "#8ab4f8", pulse: "pulseSlow",  dur: "2.4s" },
  admin_reviewing:   { label: "Being Reviewed",          color: "#c084fc", pulse: "pulseMed",   dur: "1.8s" },
  contact:           { label: "You'll Be Contacted Shortly", color: "#60a5fa", pulse: "pulseMed", dur: "2s" },
  admin_approved:    { label: "Approved",                color: "#f59e0b", pulse: "pulseMed",   dur: "1.6s" },
  ...
  completed:         { label: "Completed ✓",             color: "#38bdb2", pulse: "pulseNone",  dur: "0s"   },
...
// Mirrors the real status lifecycle end to end (see STATUS_CFG comment).
// 'contact' sits between Reviewing and Approved — the "admin is reaching
// out to confirm details" step.
const EVENT_STEPS = [
...
function Timeline({ status }) {
  ...
  return (
    <div style={{ display:"flex", alignItems:"flex-start", position:"relative", marginBottom:20 }}>
      {/* top:14 = exact vertical center of the 28px circles below. The
          circles are pinned to boxSizing:"border-box" so their border is
          included in that 28px (rather than added on top of it, which
          previously pushed their true rendered size to 32px and left this
          line 3px off-center — visible as a sliver poking out at each
          circle's edge instead of sitting cleanly behind it). */}
      <div style={{ position:"absolute", top:14, left:18, right:18, height:2, background:T.borderFaint, zIndex:0 }}/>
...
function RefImageRow({ label, imageUrl, title, type, city, price }) {
  const { isLight } = useTheme();
  const T = getTokens(isLight);
    ...
    <div style={{ marginTop:12 }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 14px", background:T.borderFaint, border:`0.5px solid ${T.border}`, borderRadius:10 }}>
        ...
        <div>
          ...
          {price > 0 && (
            <div style={{ fontSize:13, fontWeight:700, color:T.goldStrong, marginTop:4 }}>
              ₹{fmt(price)} <span style={{ fontSize:10, fontWeight:400, color:T.textFaint, textTransform:"uppercase", letterSpacing:"0.05em" }}>estimate</span>
            </div>
          )}
        </div>
      </div>
...
function VendorChips({ vendors, navigate }) {
  const { isLight } = useTheme();
  const T = getTokens(isLight);
...
/* ─── Pending add-on charges ─────────────────────────────────────────────────
   Extra charges the admin has proposed for this event (e.g. something added
   mid-event). Fetched independently from GET /api/payments/addons/:eventId
   since /api/events/my doesn't carry them. Only "pending" ones get a Pay
   button — "paid"/"cancelled" ones are just historical, shown in the
   payment ledger elsewhere. */
function AddonCharges({ addons, onPay }) {
  const { isLight } = useTheme();
  const T = getTokens(isLight);
    ...
    <div style={{ marginTop:14, display:"flex", flexDirection:"column", gap:8 }}>
      {pending.map(addon => (
        <div key={addon.id} style={{
          ...
          padding:"12px 16px", background:"rgba(212,168,67,0.08)",
          border:"1px solid rgba(212,168,67,0.3)", borderRadius:10, flexWrap:"wrap", gap:10,
        }}>
          <div>
            <div style={{ fontSize:13, fontWeight:600, color:T.textAlt }}>✚ {addon.label}</div>
            {addon.notes && <div style={{ fontSize:11, color:T.textMuted, marginTop:2 }}>{addon.notes}</div>}
          </div>
          <button
            onClick={() => onPay(addon)}
            style={{ background:"#d4a843", color:"#1a1008", border:"none", borderRadius:8, fontSize:13, fontWeight:700, padding:"8px 18px", cursor:"pointer" }}
          >
            Pay ₹{fmt(addon.amount)}
          </button>
        </div>
      ))}
    </div>
    ...
    <div style={{ borderTop:`1px solid ${T.divider}`, marginTop:16, paddingTop:20 }}>
      <div style={{ marginBottom:20 }}>
        ...
        {ev.reference_event_image && (
          <RefImageRow label="Reference event" imageUrl={ev.reference_event_image} title={ev.reference_event_title} type={ev.reference_event_type} price={ev.reference_event_price}/>
        )}
      </div>
...
function EventCard({ ev, navigate, onCancel }) {
  ...
  // Balance is only shown once admin has actually marked the event
  // "Completed" — not the moment the advance lands. Before that, the
  // client just sees the normal "Advance paid" confirmation below; the
  // remaining-balance prompt is deliberately held back until the event
  // itself is done, per the agreed flow.
  const needsBalance = !isCancelled && ev.status === "completed" && ev.payment_status === "advance_paid";
  ...
  // Pending add-on charges — fetched once per card on mount, then kept
  // live via the 'addons:update' socket event (emitted whenever admin
  // creates/cancels a charge or a payment marks one paid) so a new charge
  ...
  useEffect(() => {
    ...
    const onAddonsUpdate = (payload) => {
      if (payload?.event_id !== ev.id) return;
  ...
  const handlePayBalance = () => {
    navigate("/payments/checkout", { state: {
    ...
    <div style={{
      ...
      <div style={{ display:"flex" }}>
        ...
        <div style={{ flex:1, padding:"18px 20px" }}>
          <div className="me-card-top" style={{ display:"flex", alignItems:"flex-start", gap:12 }}>
            {ev.reference_event_image && (
            ...
            <div className="me-card-right" style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:8, flexShrink:0, marginLeft:8 }}>
              ...
              <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                {!isCancelled && ev.status !== "confirmed" && ev.status !== "completed" && (
          ...
          {!isCancelled && ev.status !== "completed" && <div style={{ marginTop:16 }}><Timeline status={ev.status}/></div>}
          <VendorChips vendors={ev.vendors} navigate={navigate}/>
          ...
          {needsBalance && (
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:14, padding:"12px 16px", background:"rgba(167,139,250,0.08)", border:"1px solid rgba(167,139,250,0.3)", borderRadius:10, flexWrap:"wrap", gap:10 }}>
              <span style={{ fontSize:13, color:T.textAlt }}>💳 Advance paid — pay remaining balance to close out this event</span>
              <button onClick={handlePayBalance} style={{ background:"#a78bfa", color:"#1a1008", border:"none", borderRadius:8, fontSize:13, fontWeight:700, padding:"9px 20px", cursor:"pointer", transition:"background 0.2s" }}>
                Pay Balance
              </button>
            </div>
          )}

          <AddonCharges addons={addons} onPay={handlePayAddon} />

          {(ev.payment_status === "advance_paid" || ev.payment_status === "fully_paid") && !isCancelled && (
            <div style={{ marginTop:12, display:"inline-flex", alignItems:"center", gap:6, fontSize:11, color:T.success, background:T.successBg, border:`0.5px solid ${T.successBorder}`, borderRadius:20, padding:"5px 12px" }}>
              ...
              {ev.payment_status === "fully_paid" ? "Fully paid" : "Advance paid · Balance due before event"}
            </div>
          )}
...
export default function MyEvents() {
  ...
  // Live updates: admin approves/rejects/updates an event, or a payment/
  // add-on/reference-price change happens server-side — this merges the
  // fresh row into local state without a manual reload or reconnect.
  useEffect(() => {
    const socket = getSocket();
  ...
  const handleCancelConfirm = async () => {
    ...
    try {
      ...
      fetchEvents();
    } catch { /* Keep the current event list when cancellation fails. */ }
    setCancelling(false);
  };
    ...
    <div style={{ minHeight:"100vh", background:T.pageBg, fontFamily:"'DM Sans',sans-serif", color:T.text, paddingTop:"70px" }}>
      <style>{`
        ...
        /* ── Responsive overrides ──────────────────────────────────────
           This page is built almost entirely with inline styles, so these
           rules use !important to win over them at the given breakpoints
           rather than duplicating the whole component in CSS. */
        @media (max-width: 640px) {
          .me-header { padding: 14px 16px !important; }
          .me-title { position: static !important; transform: none !important; font-size: 26px !important; order: -1 !important; flex-basis: 100% !important; text-align: center !important; margin-bottom: 8px !important; }
          .me-header { flex-wrap: wrap !important; row-gap: 6px !important; }
          .me-divider { margin: 0 16px !important; }
          .me-success { margin: 12px 16px 0 !important; }
          .me-tabs { padding: 0 16px !important; overflow-x: auto !important; -webkit-overflow-scrolling: touch !important; }
          .me-tabs button { margin-right: 20px !important; flex-shrink: 0 !important; }
          .me-main { padding: 20px 16px 48px !important; }
          .me-card-top { flex-wrap: wrap !important; }
          .me-card-right { align-items: flex-start !important; margin-left: 0 !important; width: 100% !important; flex-direction: row !important; justify-content: space-between !important; flex-wrap: wrap !important; row-gap: 8px !important; }
          .me-cancel-popup { padding: 20px !important; }
        }
      ...
      <header className="me-header" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"16px 32px", borderBottom:`0.5px solid ${T.borderFaint}` }}>
        ...
        <h1 className="me-title" style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:48, fontWeight:300, color:T.textAlt, margin:0, position:"absolute", left:"50%", transform:"translateX(-50%)" }}>My Events</h1>
        <button onClick={()=>navigate("/create-event")} style={{ background:T.goldStrong, color:T.goldOnDark, border:"none", borderRadius:8, fontSize:13, fontWeight:700, padding:"9px 20px", cursor:"pointer", flexShrink:0 }}>
          + New event
        </button>
      </header>

      <div className="me-divider" style={{ height:"0.5px", background:isLight ? "linear-gradient(to right,transparent,rgba(255,148,120,0.25),transparent)" : "linear-gradient(to right,transparent,rgba(201,169,110,0.12),transparent)", margin:"0 32px" }}/>

      {showSuccess && (
        <div className="me-success" style={{ margin:"16px 32px 0", padding:"12px 18px", background:T.successBg, border:`0.5px solid ${T.successBorder}`, borderRadius:10, display:"flex", alignItems:"center", gap:10, fontSize:13, color:T.success }}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="20 6 9 17 4 12"/></svg>
          Event submitted! Our team will review it within 24 hours.
        </div>
      )}

      <nav className="me-tabs" style={{ display:"flex", padding:"0 32px", background:T.navBg, gap:0, zIndex:1 }}>
        {TABS.map(({ key, list }) => (
      ...
      <main className="me-main" style={{ padding:"28px 32px 60px", maxWidth:900, margin:"0 auto" }}>
        {loading ? (
