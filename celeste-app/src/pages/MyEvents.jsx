...
function EventCard({ booking, navigate }) {
  ...
  const handlePay = () => {
    ...
    navigate('/payments/checkout', { state: { event: eventData } });
  };

  return (
    <div style={{ display:'flex', background:'#1e1a14', border:`0.5px solid ${cfg.border}`, borderRadius:14, overflow:'hidden', marginBottom:16, position:'relative' }}>
      ...
      <div style={{ flex:1, padding:'20px 24px', display:'flex', flexDirection:'column' }}>
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:12 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', background:cfg.bg, border:`0.5px solid ${cfg.border}`, borderRadius:20, padding:'5px 12px', flexShrink:0, marginLeft:16 }}>
            <GlowDot config={cfg} />
        ...
        {/* PAY BUTTON — only when confirmed and not yet paid */}
        {booking.status === 'confirmed' && booking.payment_status !== 'advance_paid' && advance > 0 && (
          <button onClick={handlePay} style={{ marginTop:8, alignSelf:'flex-start', display:'inline-flex', alignItems:'center', gap:8, background:'#6fcf97', color:'#0f1f15', border:'none', borderRadius:8, padding:'10px 20px', fontFamily:"'DM Sans',sans-serif", fontSize:12, fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase', cursor:'pointer', boxShadow:'0 4px 16px rgba(111,207,151,0.3)', transition:'background 0.2s,transform 0.15s' }}
...
function PastEventCard({ booking }) {
  ...
  return (
    <div style={{ display:'flex', background:'#1a1612', border:'0.5px solid rgba(200,175,120,0.08)', borderRadius:14, overflow:'hidden', marginBottom:16, opacity:0.75 }}>
      ...
      <div style={{ width:160, minHeight:140, flexShrink:0, background:'#1e1a14', display:'flex', alignItems:'center', justifyContent:'center', borderRight:'0.5px solid rgba(200,175,120,0.06)' }}><EventTypeIcon type={booking.event_type} /></div>
      <div style={{ flex:1, padding:'18px 22px' }}>
        <h3 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:20, fontWeight:400, color:'rgba(232,220,200,0.7)', margin:'0 0 6px' }}>{details['event']||booking.event_type||'My Event'}</h3>
...
export default function MyEvents() {
  ...
  useEffect(()=>{
    ...
    fetch(`${API}/bookings/my?email=${encodeURIComponent(user.email)}`)
      .then(r=>r.json()).then(data=>{setBookings(Array.isArray(data)?data:[]);setLoading(false);}).catch(()=>setLoading(false));
  },[user]);
        ...
        {loading?(
          ...
          upcoming.length===0
            ...
            :<div style={{width:'100%',maxWidth:860}}>
              ...
              {upcoming.map(b=><EventCard key={b.id} booking={b} navigate={navigate}/>)}
            </div>
        ):activeTab==='Past'?(
          past.length===0
