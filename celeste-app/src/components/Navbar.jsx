...
export default function Navbar({ bookmarkCount }) {
  ...
  return (
    <nav>
      ...
      {/* ── DESKTOP RIGHT SIDE — unchanged, original avatar dropdown restored ── */}
...
<div id="mobile-nav-panel" className={`mobile-menu${mobileOpen ? ' open' : ''}`} role="dialog" aria-modal="true">
  <div className="mm-header">
    <Link to="/" className="mm-logo" onClick={closeAllMenus}>Arc<em>.</em></Link>
    <div className="mm-header-actions">
      <ThemeToggle />
      <button type="button" className="mm-close" onClick={closeAllMenus} aria-label="Close menu">
  ...
  <div className="mm-scroll">
    {isLoggedIn && user && (
      <div className="mm-profile-card">
        ...
        <div className="mm-profile-info">
          <div className="mm-profile-name">{user?.name || user?.email}</div>
          <div className="mm-profile-email">{user?.email || ''}</div>
        </div>
      </div>
    )}

    <div className="mm-section">
      <Link to="/" className={`mm-link${isRouteActive('/') ? ' active' : ''}`} onClick={closeAllMenus}>
        <span className="mm-link-icon">
          <svg viewBox="0 0 16 16" fill="none"><path d="M2 7.5L8 2l6 5.5M3.5 6.5V14h9V6.5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/></svg>
        </span>
        <span>Home</span>
      </Link>
      ...
      <div className="mm-accordion">
        ...
        <div className={`mm-accordion-panel${mobileExploreOpen ? ' open' : ''}`}>
          <div className="mm-accordion-panel-inner">
            <Link to="/explore" className="mm-subitem" onClick={closeAllMenus}>All Events</Link>
            {featuredEventTypes.map((type) => (
              <button key={type} type="button" className="mm-subitem" onClick={() => handleTypeClick(type)}>{type}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="mm-accordion">
        ...
        <div className={`mm-accordion-panel${mobileServicesOpen ? ' open' : ''}`}>
          <div className="mm-accordion-panel-inner">
            {SERVICES.map((col) => (
              <div key={col.col} className="mm-service-group">
                <div className="mm-service-group-label">
                  <span className="mm-service-dot" style={{ background: col.iconBg }}>{col.icon}</span>
                  {col.col}
                </div>
                {col.items.map((item) => (
                  <Link key={item.name} to={item.path || '/services'} className="mm-subitem mm-service-item" onClick={closeAllMenus}>
                    <span>{item.name}</span>
                    {item.tag && <span className={`nsm-pill nsm-pill-${item.tag.type}`}>{item.tag.label}</span>}
                  </Link>
                ))}
              </div>
            ))}
      ...
      <Link to="/about" className={`mm-link${isRouteActive('/about') ? ' active' : ''}`} onClick={closeAllMenus}>
        <span className="mm-link-icon">
          <svg viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="6.25" stroke="currentColor" strokeWidth="1.3"/><path d="M8 7.2v3.6M8 5.2v.1" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg>
        </span>
        <span>About</span>
      </Link>
      <Link to="/contact" className={`mm-link${isRouteActive('/contact') ? ' active' : ''}`} onClick={closeAllMenus}>
        <span className="mm-link-icon">
          <svg viewBox="0 0 16 16" fill="none"><path d="M2 4.5l6 4 6-4M2.5 3h11a.5.5 0 01.5.5v9a.5.5 0 01-.5.5h-11a.5.5 0 01-.5-.5v-9a.5.5 0 01.5-.5z" stroke="currentColor" strokeWidth="1.3" strokeLinejoin="round"/></svg>
        </span>
        <span>Contacts</span>
      </Link>
    </div>
    ...
    {!isLoggedIn ? (
      <div className="mm-auth">
        <Link to="/signup" className="btn-fill mm-auth-btn" onClick={closeAllMenus}>Sign Up</Link>
        <Link to="/login" className="btn-outline mm-auth-btn" onClick={closeAllMenus}>Log In</Link>
      </div>
    ) : (
      <>
        <div className="mm-account-card">
          <div className="mm-account-label">Account</div>
          <Link to="/my-events" className="mm-account-item" onClick={closeAllMenus}>
            <span className="cd-icon"><svg viewBox="0 0 16 16" fill="none"><rect x="2" y="3" width="12" height="11" rx="2" stroke="currentColor" strokeWidth="1.3"/><path d="M5 2v2M11 2v2M2 7h12" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg></span>
            My Events
          </Link>
          <Link to="/create-events" className="mm-account-item" onClick={closeAllMenus}>
            <span className="cd-icon"><svg viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="6" stroke="currentColor" strokeWidth="1.3"/><path d="M8 5v6M5 8h6" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg></span>
            Create Event
          </Link>
          <BookmarkMenuItem count={bookmarkCount} label="Saved Boards" className="mm-account-item" onClick={closeAllMenus} />
          <button type="button" className="mm-account-item" onClick={() => { closeAllMenus(); navigate("/payments"); }}>
            <span className="cd-icon"><svg viewBox="0 0 16 16" fill="none"><rect x="1.5" y="4" width="13" height="9" rx="1.5" stroke="currentColor" strokeWidth="1.3"/><path d="M1.5 7h13" stroke="currentColor" strokeWidth="1.3"/></svg></span>
            Payments
          </button>
          <button type="button" className="mm-account-item" onClick={closeAllMenus}>
            <span className="cd-icon"><svg viewBox="0 0 16 16" fill="none"><circle cx="8" cy="8" r="2" stroke="currentColor" strokeWidth="1.3"/><path d="M8 1.5v2M8 12.5v2M1.5 8h2M12.5 8h2M3.4 3.4l1.4 1.4M11.2 11.2l1.4 1.4M12.6 3.4l-1.4 1.4M4.8 11.2l-1.4 1.4" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg></span>
            Settings
          </button>
        </div>

        <button type="button" className="mm-signout" onClick={() => { closeAllMenus(); signOut(); }}>
          <svg viewBox="0 0 16 16" fill="none"><path d="M6 14H3a1 1 0 01-1-1V3a1 1 0 011-1h3M10 11l3-3-3-3M13 8H6" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/></svg>
          Sign Out
        </button>
      </>
    )}

    <div className="mm-socials"><SocialLinks /></div>
  </div>
</div>
    </nav>
  );
