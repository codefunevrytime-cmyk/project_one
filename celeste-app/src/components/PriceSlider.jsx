...
export default function PriceSlider({ min, max, value, onChange }) {
  ...
  const clamp = useCallback((val) => Math.max(min, Math.min(max, val)), [min, max]);
  ...
  const handleTrackClick = useCallback((e) => {
    ...
    setLocalValue(next);
    onChange(next);
  }, [min, max, localValue, onChange, clamp]);
  ...
  const handleDrag = (index) => (e) => {
    ...
    const move = (ev) => {
      const rect = trackRef.current.getBoundingClientRect();
      const percent = Math.max(0, Math.min(1, (getClientX(ev) - rect.left) / rect.width));
      const raw = clamp(Math.round(min + percent * (max - min)));
    ...
    // Touch: {passive:false} so preventDefault (above) can actually stop
    // the page from scrolling while a thumb is being dragged.
    ...
    <div style={{ padding: '8px 0 4px' }}>
      <div
        ...
        style={{
          ...
          borderRadius: 2, cursor: 'pointer', margin: '20px 0',
        }}
      >
        <div style={{
        ...
        {[0, 1].map((i) => (
          <div
            ...
            style={{
              ...
              // Visible dot stays 16px, but the actual hit target is
              // padded out to ~28px via a transparent halo — 16px alone
              // is well under the ~24px minimum comfortable touch target.
