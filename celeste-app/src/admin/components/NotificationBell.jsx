...
const TYPE_META = {
  ...
  payment:    { icon: '💳',   color: '#15803d', label: 'Payment Received' },
  payout:     { icon: '💸',   color: '#d97706', label: 'Payout Due' },
  deposit:    { icon: '🛡️',   color: '#b91c1c', label: 'Deposit' },
};
...
// vendor-payouts/admin returns a bare array (confirmed from AdminVendorPayouts.jsx),
// so safeJson works fine there. payments/deposit/admin/all also returns a bare
// array (confirmed from AdminVendorDeposits.jsx) — same helper covers both.
...
export default function NotificationBell({ onNavigate }) {
  ...
  const fetchAll = useCallback(async () => {
    ...
    // New event requests (status still 'pending', i.e. not yet reviewed by admin)
    ...
    // ── NEW: client payments received ───────────────────────────────────
    // Events whose payment has actually cleared (advance or full). This is
    // a best-effort read from the event record itself since there's no
    // dedicated payment-events endpoint — `updated_at` is assumed to move
    // when payment_status changes; if your events table doesn't bump that
    // column on payment, swap it for whatever timestamp field your
    // `/api/events/admin/all` payload actually sets on payment (e.g. an
    // `advance_paid_at` / `paid_at` column) so this doesn't just show the
    // event's original creation time.
    events
      .filter(e => e.payment_status === 'advance_paid' || e.payment_status === 'fully_paid')
      .forEach(e => {
        list.push({
          id: `payment_${e.id}_${e.payment_status}`,
          type: 'payment',
          title: e.payment_status === 'fully_paid' ? 'Balance payment received' : 'Advance payment received',
          desc: `${e.client_name} paid for "${e.event_name || e.event_type || 'an event'}"${e.budget_estimate ? ` · ₹${Number(e.budget_estimate).toLocaleString('en-IN')} total` : ''}`,
          time: e.updated_at || e.created_at,
          tab: 'events',
        });
      });

    // ── NEW: vendor payouts still owed (money admin needs to send out) ──
    payouts.filter(p => p.status === 'pending').forEach(p => {
      list.push({
        id: `payout_${p.id}`,
        type: 'payout',
        title: 'Vendor payout pending',
        desc: `₹${(Number(p.amount) / 100).toLocaleString('en-IN')} owed to ${p.vendor_name || `Vendor #${p.vendor_id}`} for ${p.event_name || 'a booking'}`,
        time: p.created_at,
        tab: 'payouts',
      });
    });

    // ── NEW: vendor trials that ended without a deposit collected ───────
    deposits.filter(d => d.trial_expired).forEach(d => {
      list.push({
        id: `deposit_trial_${d.vendor_id}`,
        type: 'deposit',
        title: 'Vendor trial ended — deposit not collected',
        desc: `${d.vendor_name}'s 2-month trial ended ${d.days_since_trial_expired ?? '?'} day${d.days_since_trial_expired === 1 ? '' : 's'} ago and still needs their ₹1,000 deposit collected`,
        time: d.trial_ends_at,
        tab: 'deposits',
      });
    });
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      {open && (
        <div ref={panelRef} className="admin-notif-dropdown" style={{
